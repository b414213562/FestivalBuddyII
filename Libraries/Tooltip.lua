...
function ProcessToolTip(SENDER, MESSAGE)
    ...
    local wasTooltipShown = false;
    tempTimer = Turbine.UI.Control();
    tempTimer.Update = function()
        local CURRENTTIME = Turbine.Engine.GetGameTime();
        if ((CURRENTTIME > (ENTERTIME + TOOLTIPDELAY)) and (not wasTooltipShown)) then
            wasTooltipShown = true;
            DrawToolTip(SENDER,MESSAGE);
            if tempTimer ~= nil then tempTimer:SetWantsUpdates(false) end;
            tempTimer = nil;
        end
    end
